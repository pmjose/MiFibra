...
CREATE OR REPLACE SEMANTIC VIEW MIFIBRA_FINANCE_SEMANTIC_VIEW
    ...
    FACTS (
        TRANSACTIONS.TRANSACTION_AMOUNT AS AMOUNT COMMENT 'Transaction amount in Soles'
    )
    DIMENSIONS (
        TRANSACTIONS.TRANSACTION_DATE AS DATE COMMENT 'Transaction date',
        ACCOUNTS.ACCOUNT_NAME AS ACCOUNT_NAME COMMENT 'Account name',
        DEPARTMENTS.DEPARTMENT_NAME AS DEPARTMENT_NAME COMMENT 'Department name',
        VENDORS.VENDOR_NAME AS VENDOR_NAME COMMENT 'Vendor name',
        PRODUCTS.PRODUCT_NAME AS PRODUCT_NAME COMMENT 'Product name',
        CUSTOMERS.CUSTOMER_NAME AS CUSTOMER_NAME COMMENT 'Customer name',
        CUSTOMERS.INDUSTRY AS INDUSTRY COMMENT 'Industry',
        CUSTOMERS.VERTICAL AS VERTICAL COMMENT 'Customer segment',
        CUSTOMERS.LATITUDE AS LATITUDE COMMENT 'Customer latitude',
        CUSTOMERS.LONGITUDE AS LONGITUDE COMMENT 'Customer longitude',
        CUSTOMERS.CITY AS CITY COMMENT 'Customer city',
        CUSTOMERS.COUNTY AS COUNTY COMMENT 'Customer county'
    )
    METRICS (
        TRANSACTIONS.TOTAL_AMOUNT AS SUM(TRANSACTIONS.AMOUNT) COMMENT 'Total amount'
    )
...
CREATE OR REPLACE SEMANTIC VIEW MIFIBRA_SALES_SEMANTIC_VIEW
    ...
    FACTS (
        SALES.SALE_AMOUNT AS AMOUNT COMMENT 'Sale amount in Soles',
        SALES.SALE_UNITS AS UNITS COMMENT 'Units sold'
    )
    DIMENSIONS (
        CUSTOMERS.CUSTOMER_NAME AS CUSTOMER COMMENT 'Customer name',
        CUSTOMERS.INDUSTRY AS INDUSTRY COMMENT 'Customer industry',
        PRODUCTS.PRODUCT_NAME AS PRODUCT COMMENT 'Product name',
        CATEGORIES.CATEGORY_NAME AS CATEGORY COMMENT 'Category name',
        CATEGORIES.VERTICAL AS VERTICAL COMMENT 'Category vertical',
        REGIONS.REGION_NAME AS REGION COMMENT 'Region name',
        REGIONS.LATITUDE AS LATITUDE COMMENT 'Region latitude',
        REGIONS.LONGITUDE AS LONGITUDE COMMENT 'Region longitude',
        SALES.DATE AS DATE COMMENT 'Sale date',
        SALES_REPS.REP_NAME AS REP COMMENT 'Sales rep name',
        VENDORS.VENDOR_NAME AS VENDOR COMMENT 'Vendor name'
    )
    METRICS (
        SALES.REVENUE AS SUM(SALES.AMOUNT) COMMENT 'Total revenue',
        SALES.TOTALUNITS AS SUM(SALES.UNITS) COMMENT 'Total units sold'
    )
...
CREATE OR REPLACE SEMANTIC VIEW MIFIBRA_MARKETING_SEMANTIC_VIEW
    ...
    FACTS (
        CAMPAIGNS.CAMPAIGN_SPEND AS SPEND COMMENT 'Marketing spend in Soles',
        CAMPAIGNS.CAMPAIGN_LEADS AS LEADS COMMENT 'Leads generated',
        CAMPAIGNS.CAMPAIGN_IMPRESSIONS AS IMPRESSIONS COMMENT 'Impressions',
        OPPORTUNITIES.OPP_REVENUE AS REVENUE COMMENT 'Opportunity revenue'
    )
    DIMENSIONS (
        ACCOUNTS.ACCOUNT_NAME AS ACCOUNT COMMENT 'Account name',
        ACCOUNTS.INDUSTRY AS INDUSTRY COMMENT 'Industry',
        CAMPAIGN_DETAILS.CAMPAIGN_NAME AS CAMPAIGN COMMENT 'Campaign name',
        CAMPAIGN_DETAILS.OBJECTIVE AS OBJECTIVE COMMENT 'Campaign objective',
        CHANNELS.CHANNEL_NAME AS CHANNEL COMMENT 'Marketing channel',
        CONTACTS.FIRST_NAME AS FIRSTNAME COMMENT 'Contact first name',
        CONTACTS.LAST_NAME AS LASTNAME COMMENT 'Contact last name',
        OPPORTUNITIES.OPPORTUNITY_NAME AS OPPORTUNITY COMMENT 'Opportunity name',
        OPPORTUNITIES.STAGE_NAME AS STAGE COMMENT 'Opportunity stage',
        PRODUCTS.PRODUCT_NAME AS PRODUCT COMMENT 'Product name',
        REGIONS.REGION_NAME AS REGION COMMENT 'Region name',
        CAMPAIGNS.DATE AS DATE COMMENT 'Campaign date'
    )
    METRICS (
        CAMPAIGNS.TOTALSPEND AS SUM(CAMPAIGNS.SPEND) COMMENT 'Total spend',
        CAMPAIGNS.TOTALLEADS AS SUM(CAMPAIGNS.LEADS) COMMENT 'Total leads',
        OPPORTUNITIES.TOTALREVENUE AS SUM(OPPORTUNITIES.REVENUE) COMMENT 'Total revenue'
    )
...
CREATE OR REPLACE SEMANTIC VIEW MIFIBRA_HR_SEMANTIC_VIEW
    ...
    FACTS (
        HR_RECORDS.EMP_SALARY AS SALARY COMMENT 'Employee salary',
        HR_RECORDS.EMP_ATTRITION AS ATTRITION COMMENT 'Attrition flag'
    )
    DIMENSIONS (
        DEPARTMENTS.DEPARTMENT_NAME AS DEPARTMENT COMMENT 'Department name',
        EMPLOYEES.EMPLOYEE_NAME AS EMPLOYEE COMMENT 'Employee name',
        EMPLOYEES.GENDER AS GENDER COMMENT 'Gender',
        EMPLOYEES.HIRE_DATE AS HIREDATE COMMENT 'Hire date',
        JOBS.JOB_TITLE AS JOBTITLE COMMENT 'Job title',
        JOBS.JOB_LEVEL AS JOBLEVEL COMMENT 'Job level',
        LOCATIONS.LOCATION_NAME AS LOCATION COMMENT 'Location name',
        LOCATIONS.CITY AS CITY COMMENT 'City',
        LOCATIONS.LATITUDE AS LATITUDE COMMENT 'Latitude',
        LOCATIONS.LONGITUDE AS LONGITUDE COMMENT 'Longitude',
        HR_RECORDS.DATE AS DATE COMMENT 'Record date'
    )
    METRICS (
        HR_RECORDS.TOTALSALARY AS SUM(HR_RECORDS.SALARY) COMMENT 'Total salary cost',
        HR_RECORDS.AVGSALARY AS AVG(HR_RECORDS.SALARY) COMMENT 'Average salary',
        HR_RECORDS.ATTRITIONCOUNT AS SUM(HR_RECORDS.ATTRITION) COMMENT 'Attrition count'
    )
...
CREATE OR REPLACE SEMANTIC VIEW MIFIBRA_INFRASTRUCTURE_SEMANTIC_VIEW
    ...
    FACTS (
        NETWORK_NODES.NODE_CAPACITY AS CAPACITY COMMENT 'Capacity in Gbps',
        NETWORK_NODES.NODE_SUBSCRIBERS AS SUBSCRIBERS COMMENT 'Active subscribers',
        NETWORK_NODES.NODE_UPTIME AS UPTIME COMMENT 'Uptime percentage',
        NETWORK_NODES.NODE_HOUSEHOLDS AS HOUSEHOLDS COMMENT 'Households passed',
        NETWORK_NODES.NODE_FIBER AS FIBERKM COMMENT 'Fiber kilometers'
    )
    DIMENSIONS (
        NETWORK_NODES.CITY_NAME AS CITY COMMENT 'City name',
        NETWORK_NODES.DEPARTMENT AS DEPARTMENT COMMENT 'Department',
        NETWORK_NODES.NODE_TYPE AS NODETYPE COMMENT 'Node type',
        NETWORK_NODES.STATUS AS STATUS COMMENT 'Node status',
        NETWORK_NODES.TECHNOLOGY AS TECHNOLOGY COMMENT 'Fiber technology',
        NETWORK_NODES.LATITUDE AS NODELATITUDE COMMENT 'Node latitude',
        NETWORK_NODES.LONGITUDE AS NODELONGITUDE COMMENT 'Node longitude',
        NETWORK_NODES.UTILIZATION_PCT AS UTILIZATION COMMENT 'Utilization percentage',
        NETWORK_NODES.PENETRATION_PCT AS PENETRATION COMMENT 'Market penetration',
        REGIONS.REGION_NAME AS REGION COMMENT 'Region name',
        REGIONS.LATITUDE AS REGIONLATITUDE COMMENT 'Region latitude',
        REGIONS.LONGITUDE AS REGIONLONGITUDE COMMENT 'Region longitude'
    )
    METRICS (
        NETWORK_NODES.TOTALCAPACITY AS SUM(NETWORK_NODES.CAPACITY) COMMENT 'Total capacity',
        NETWORK_NODES.TOTALSUBSCRIBERS AS SUM(NETWORK_NODES.SUBSCRIBERS) COMMENT 'Total subscribers',
        NETWORK_NODES.TOTALFIBER AS SUM(NETWORK_NODES.FIBERKM) COMMENT 'Total fiber km',
        NETWORK_NODES.AVGUPTIME AS AVG(NETWORK_NODES.UPTIME) COMMENT 'Average uptime'
    )
